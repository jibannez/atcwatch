...
# tuned to work with our V3 watch and with framebuffer font rendering
# No longer used as st7789py integrates the relevant bits from this 
# module and adds rendering of large fonts.
...
class ST77xx:
    ...
    def __init__(self, spi, width, height, reset, dc, cs=None, backlight=None,
        ...
        self.spi = spi
        if spi is None:
            self.spi = machine.SPI(2,baudrate=32000000, polarity=1, phase=0, bits=8, firstbit=0, sck=machine.Pin(18,machine.Pin.OUT),mosi=machine.Pin(19,machine.Pin.OUT))
            self.spi.init()
        self.reset = reset
        self.dc = dc
        self.cs = cs
...
class ST7789font(ST7789):
    ...
    def text(self, s, x, y, fg, bg):
        ...
        if text_mem > len(self.buf):
            raise ValueError("buffer too small %d vs %d" % (text_mem, len(self.buf))  )
...
class ST7789fontsmall(ST7789):
    def init(self, *arg, **kw):
        super().init(*arg,**kw)
        self.fontinit()
        #self.testimg()
        
    def fontinit(self, width=10, height=16, font_name='font5x8.bin'):
